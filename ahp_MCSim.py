...
import threading # NEW: For running the simulation in a separate thread
import queue     # NEW: For communicating between threads

# --- 1. AHP Engine & Database Layer (With updated simulation function) ---

# ... (RI_TABLE, calculate_priority_vector, calculate_consistency are identical) ...
RI_TABLE = {1: 0, 2: 0, 3: 0.58, 4: 0.90, 5: 1.12, 6: 1.24, 7: 1.32, 8: 1.41, 9: 1.45, 10: 1.49}
def calculate_priority_vector(matrix: np.ndarray) -> np.ndarray:
    col_sums = matrix.sum(axis=0); normalized_matrix = matrix / col_sums; return normalized_matrix.mean(axis=1)
...
# MODIFIED: The simulation now accepts a queue to report progress
def run_monte_carlo_simulation(decision_data, n_simulations=1000, uncertainty_factor=0.1, progress_queue=None):
    criteria = decision_data['criteria']; alternatives = decision_data['alternatives']
    all_final_scores = []
    
    for i in range(n_simulations):
        # --- Report progress periodically ---
        if progress_queue and (i % 20 == 0 or i == n_simulations - 1):
            progress = int((i + 1) / n_simulations * 100)
            progress_queue.put(f"progress_{progress}")
    ...
    # NEW: Put the final result onto the queue
    if progress_queue:
...
# --- 2. GUI Application Layer (With Threading and Progress Bar) ---
...
class AHP_GUI(ThemedTk):
    ...
    def __init__(self):
        ...
        self.geometry("900x700"); self.db = AHPDatabase(); self.decision_data = {}; self.comparison_queue = []
        self.judgments = []; self.container = ttk.Frame(self); self.container.pack(fill="both", expand=True, padx=10, pady=10)
    ...
    def _load_selected_decision(self):
        ...
        self.decision_data = {'goal': selected_goal, 'criteria': list(crit_name_map.keys()), 'alternatives': list(alt_name_map.keys()), 'decision_id': decision_id}
        crit_judgments = self.db.get_criteria_judgments(decision_id); self.decision_data['criteria_matrix'] = self._build_matrix_from_judgments(crit_name_map, crit_id_map, crit_judgments)
    ...
    def _build_matrix_from_judgments(self, name_map, id_map, judgments):
        n = len(name_map); matrix = np.ones((n, n)); item_list = list(name_map.keys())
        id_to_idx = {name_map[name]: i for i, name in enumerate(item_list)}
        for id1, id2, value in judgments: i, j = id_to_idx[id_map[id1]], id_to_idx[id_map[id2]]; matrix[i, j] = value; matrix[j, i] = 1 / value
        return matrix
    def _start_comparisons(self):
        ...
        self.decision_data = {'goal': goal, 'criteria': criteria, 'alternatives': alternatives, 'decision_id': self.db.create_decision(goal, criteria, alternatives)}
    ...
    def _process_next_comparison(self):
        if not self.comparison_queue: self._calculate_and_show_results()
    ...
    # MODIFIED: Includes progress bar and threading logic
    def _calculate_and_show_results(self):
        self.geometry("900x700"); crit_weights = self.decision_data['criteria_weights']
        ...
        ttk.Label(top_frame, text=f"Final Results for '{self.decision_data['goal']}'", font=("Helvetica", 16, "bold")).pack(pady=10)
        ...
        ranking_tree = ttk.Treeview(ranking_frame, columns=("score"), show="headings")
        ranking_tree.heading("#0", text="Alternative"); ranking_tree.heading("score", text="Final Score"); ranking_tree.column("score", anchor="center")
        results = sorted(zip(alternatives, final_scores), key=lambda x: x[1], reverse=True)
        for alt, score in results: ranking_tree.insert("", "end", text=alt, values=(f"{score:.4f}",))
        ...
        ttk.Label(sensitivity_frame, text="Uncertainty:").grid(row=0, column=0, padx=(0, 5), sticky="w")
        self.uncertainty_slider = ttk.Scale(sensitivity_frame, from_=0.01, to=0.5, orient="horizontal", length=120)
        self.uncertainty_slider.set(0.1); self.uncertainty_slider.grid(row=0, column=1)
        self.uncertainty_label = ttk.Label(sensitivity_frame, text="0.100"); self.uncertainty_label.grid(row=0, column=2, padx=5, sticky="w")
        self.uncertainty_slider.config(command=lambda v: self.uncertainty_label.config(text=f"{float(v):.3f}"))
        
        ttk.Label(sensitivity_frame, text="Simulations:").grid(row=1, column=0, padx=(0, 5), pady=5, sticky="w")
        self.sim_count_entry = ttk.Entry(sensitivity_frame, width=10); self.sim_count_entry.insert(0, "1000")
        self.sim_count_entry.grid(row=1, column=1, sticky="w")
        
        self.run_sensitivity_button = ttk.Button(sensitivity_frame, text="Run Sensitivity Analysis", command=self._run_and_display_sensitivity, style="Accent.TButton") # MODIFIED
        self.run_sensitivity_button.grid(row=0, column=3, rowspan=2, padx=10, ipady=5)
        
        # NEW: Progress bar widget
        self.progress_bar = ttk.Progressbar(sensitivity_frame, orient='horizontal', length=150, mode='determinate')
        self.progress_bar.grid(row=2, column=0, columnspan=4, sticky="ew", pady=(10, 0))
    ...
    def _draw_score_plot(self, results, errors=None):
        ...
        if errors is not None:
            error_values = [errors[label] for label in plot_labels]
            ax.barh(plot_labels, plot_scores, xerr=error_values, capsize=5, color='skyblue', ecolor='gray')
        else: ax.barh(plot_labels, plot_scores, color='skyblue')
        ax.set_xlabel('Final Score'); ax.set_title('Alternatives Final Score Comparison'); fig.tight_layout()
        canvas = FigureCanvasTkAgg(fig, master=self.plot_frame); canvas.draw(); canvas.get_tk_widget().pack(fill='both', expand=True)
    
    # MODIFIED: Now manages threading
    def _run_and_display_sensitivity(self):
        try:
            uncertainty = self.uncertainty_slider.get()
            n_sims = int(self.sim_count_entry.get())
            if n_sims <= 0: raise ValueError
        except (ValueError, TypeError):
            messagebox.showerror("Input Error", "Please enter a valid, positive whole number for the simulation count.")
            return

        self.run_sensitivity_button.config(state="disabled") # Disable button
        self.progress_bar["value"] = 0 # Reset progress bar
        
        # NEW: Setup queue and worker thread
        self.progress_queue = queue.Queue()
        self.simulation_thread = threading.Thread(
            target=run_monte_carlo_simulation,
            args=(self.decision_data, n_sims, uncertainty, self.progress_queue)
        )
    ...
    # NEW: Method to poll the queue from the main GUI thread
    def _check_simulation_progress(self):
        try:
            message = self.progress_queue.get_nowait()
            if isinstance(message, str) and message.startswith("progress_"):
                progress_val = int(message.split("_")[1])
                self.progress_bar["value"] = progress_val
                self.after(100, self._check_simulation_progress) # Continue polling
                ...
                self._draw_score_plot(results, errors)
                
                self.run_sensitivity_button.config(state="normal") # Re-enable button
                self.progress_bar["value"] = 0 # Hide/reset progress bar
                messagebox.showinfo("Success", "Sensitivity analysis complete. Plot updated.")
        except queue.Empty:
            # If the queue is empty, it means the thread is still working.
            # Continue polling.
            self.after(100, self._check_simulation_progress)
