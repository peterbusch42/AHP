    ...
    for i in range(n_simulations):
        ...
        for r in range(len(criteria)):
            for c in range(r + 1, len(criteria)):
                original_judgment = crit_matrix_orig[r, c]
                simulated_judgment = np.clip(norm.rvs(loc=original_judgment, scale=uncertainty_factor), 1/9, 9)
                crit_matrix_sim[r, c] = simulated_judgment
                crit_matrix_sim[c, r] = 1 / simulated_judgment
        ...
        for crit_name in criteria:
            ...
            for r in range(len(alternatives)):
                for c in range(r + 1, len(alternatives)):
                    original_judgment = alt_matrix_orig[r, c]
                    simulated_judgment = np.clip(norm.rvs(loc=original_judgment, scale=uncertainty_factor), 1/9, 9)
                    alt_matrix_sim[r, c] = simulated_judgment
                    alt_matrix_sim[c, r] = 1 / simulated_judgment
            sim_alt_weights = calculate_priority_vector(alt_matrix_sim)
            sim_alt_weights_matrix.append(sim_alt_weights)
            
        final_scores_sim = np.array(sim_alt_weights_matrix).T @ sim_crit_weights
        all_final_scores.append(final_scores_sim)
        
    all_final_scores = np.array(all_final_scores)
    mean_scores = np.mean(all_final_scores, axis=0)
    std_dev_scores = np.std(all_final_scores, axis=0)
    
    if progress_queue:
        progress_queue.put(('result', mean_scores, std_dev_scores))

def run_one_way_sensitivity_analysis(decision_data, comparison_set, item1_name, item2_name, num_steps=50):
    judgment_range = np.linspace(1/9, 9, num_steps)
    all_alternatives = decision_data['alternatives']
    results = {alt: [] for alt in all_alternatives}

    crit_weights_base = decision_data['criteria_weights'].copy()
    alt_weights_base = {k: v.copy() for k, v in decision_data['alternative_weights'].items()}

    for value in judgment_range:
        temp_crit_weights = crit_weights_base
        temp_alt_weights = alt_weights_base.copy()

        if comparison_set == "Criteria":
            matrix = decision_data['criteria_matrix'].copy()
            items = decision_data['criteria']
            item_map = {name: i for i, name in enumerate(items)}
            i, j = item_map[item1_name], item_map[item2_name]
            matrix[i, j] = value
            matrix[j, i] = 1 / value
            temp_crit_weights = calculate_priority_vector(matrix)
        else:
            context_crit = comparison_set.replace("Alternatives vs. ", "")
            matrix = decision_data['alternative_matrices'][context_crit].copy()
            items = decision_data['alternatives']
            item_map = {name: i for i, name in enumerate(items)}
            i, j = item_map[item1_name], item_map[item2_name]
            matrix[i, j] = value
            matrix[j, i] = 1 / value
        ...
        for i, alt_name in enumerate(all_alternatives):
            results[alt_name].append(final_scores[i])
...
class AHPDatabase:
    ...
    def _create_tables(self):
        ...
        self.cursor.execute("CREATE TABLE IF NOT EXISTS criteria (id INTEGER PRIMARY KEY, decision_id INTEGER NOT NULL, name TEXT NOT NULL, FOREIGN KEY (decision_id) REFERENCES decisions (id) ON DELETE CASCADE)")
        self.cursor.execute("CREATE TABLE IF NOT EXISTS alternatives (id INTEGER PRIMARY KEY, decision_id INTEGER NOT NULL, name TEXT NOT NULL, FOREIGN KEY (decision_id) REFERENCES decisions (id) ON DELETE CASCADE)")
        self.cursor.execute("CREATE TABLE IF NOT EXISTS criteria_judgments (decision_id INTEGER NOT NULL, crit1_id INTEGER NOT NULL, crit2_id INTEGER NOT NULL, value REAL NOT NULL, FOREIGN KEY (decision_id) REFERENCES decisions (id) ON DELETE CASCADE)")
        self.cursor.execute("CREATE TABLE IF NOT EXISTS alternative_judgments (decision_id INTEGER NOT NULL, criterion_id INTEGER NOT NULL, alt1_id INTEGER NOT NULL, alt2_id INTEGER NOT NULL, value REAL NOT NULL, FOREIGN KEY (decision_id) REFERENCES decisions (id) ON DELETE CASCADE)")
        self.conn.commit()
    ...
    def remove_criterion(self, decision_id, name):
        ...
        if crit_id:
            self.cursor.execute("DELETE FROM criteria WHERE id = ?", (crit_id,))
            self.cursor.execute("DELETE FROM criteria_judgments WHERE crit1_id = ? OR crit2_id = ?", (crit_id, crit_id))
            self.cursor.execute("DELETE FROM alternative_judgments WHERE criterion_id = ?", (crit_id,))
    def remove_alternative(self, decision_id, name):
        ...
        if alt_id:
            self.cursor.execute("DELETE FROM alternatives WHERE id = ?", (alt_id,))
            self.cursor.execute("DELETE FROM alternative_judgments WHERE alt1_id = ? OR alt2_id = ?", (alt_id, alt_id))
...
class AHP_GUI(ThemedTk):
    def __init__(self):
        ...
        self.geometry("950x750"); self.db = AHPDatabase(); self.decision_data = {}; self.comparison_queue = []
    ...
    def _show_load_frame(self):
        ...
        self.listbox = tk.Listbox(frame, height=10); [self.listbox.insert(tk.END, goal) for goal in self.decision_map.keys()]; self.listbox.pack(fill="x", padx=20, pady=(0, 10))
    ...
    def _finalize_current_comparison_set(self):
        comp_details = self.current_comp_details; items = comp_details['items']
        decision_id = self.decision_data['decision_id']; save_method = self.db.update_judgments if self.edit_mode else self.db.save_judgments
    ...
    def _calculate_and_show_results(self):
        ...
        top_frame = ttk.Frame(self.container); top_frame.pack(fill="x")
        ...
        self.hierarchy_tree.heading("#0", text="Component"); self.hierarchy_tree.heading("weight", text="Weight"); self.hierarchy_tree.column("weight", width=80, anchor="center")
        goal_node = self.hierarchy_tree.insert("", "end", text=self.decision_data['goal'], open=True)
        ...
        ranking_frame = ttk.Frame(notebook, padding=10); notebook.add(ranking_frame, text='Final Ranking'); self.ranking_tree = ttk.Treeview(ranking_frame, columns=("score"), show="headings")
        ...
        self.cr_tree.column("cr_value", anchor="center"); self.cr_tree.column("status", anchor="center"); self.cr_tree.tag_configure('good', background='#d4edda'); self.cr_tree.tag_configure('bad', background='#f8d7da')
        ...
        ttk.Button(bottom_frame, text="<< Main Menu", command=self._show_welcome_frame).grid(row=0, column=0, padx=5, sticky="w")
        ttk.Button(bottom_frame, text="Edit Definition", command=self._show_edit_definition_frame).grid(row=0, column=1, padx=5, sticky="w")
        ttk.Button(bottom_frame, text="Edit Judgments", command=self._start_editing).grid(row=0, column=2, padx=5, sticky="w")
        ttk.Button(bottom_frame, text="Export Report", command=self._export_results).grid(row=0, column=3, padx=5, sticky="w")

        mc_frame = ttk.LabelFrame(bottom_frame, text="Monte Carlo Analysis"); mc_frame.grid(row=0, column=4, padx=10, sticky="e")
        ttk.Label(mc_frame, text="Uncertainty:").grid(row=0, column=0, padx=(5,0), sticky="w"); self.uncertainty_slider = ttk.Scale(mc_frame, from_=0.01, to=0.5, orient="horizontal", length=100); self.uncertainty_slider.set(0.1); self.uncertainty_slider.grid(row=0, column=1); self.uncertainty_label = ttk.Label(mc_frame, text="0.100"); self.uncertainty_label.grid(row=0, column=2); self.uncertainty_slider.config(command=lambda v: self.uncertainty_label.config(text=f"{float(v):.3f}"))
        ttk.Label(mc_frame, text="Simulations:").grid(row=1, column=0, padx=(5,0), sticky="w"); self.sim_count_entry = ttk.Entry(mc_frame, width=8); self.sim_count_entry.insert(0, "1000"); self.sim_count_entry.grid(row=1, column=1, sticky="w")
        self.run_mc_button = ttk.Button(mc_frame, text="Run", command=self._run_and_display_mc, style="Accent.TButton"); self.run_mc_button.grid(row=0, column=3, rowspan=2, padx=5, ipady=5)
        self.mc_progress_bar = ttk.Progressbar(mc_frame, orient='horizontal', length=200, mode='determinate'); self.mc_progress_bar.grid(row=2, column=0, columnspan=4, sticky="ew", pady=5, padx=5)
    ...
    def _populate_sensitivity_tab(self):
        ...
        ttk.Button(controls_frame, text="Run Analysis", command=self._run_one_way_sensitivity).grid(row=1, column=2, rowspan=2, padx=10)
